## RuoYi-Vue-Plus 包名修改器

> 支持一键修改 RuoYi-Vue-Plus 包名、模块名和项目配置
...
- 图形用户界面（JavaFX），侧边栏导航，现代化深色侧边栏设计
- 支持打包成 EXE 可执行文件

## 界面预览

<!-- 截图：主界面 - 源文件配置页 -->
![源文件配置页](docs/screenshots/source-page.png)

<!-- 截图：包名配置页 -->
![包名配置页](docs/screenshots/package-page.png)

<!-- 截图：模块映射配置页 -->
![模块映射配置页](docs/screenshots/module-page.png)

<!-- 截图：处理日志页 -->
![处理日志页](docs/screenshots/log-page.png)
...
启动后在左侧导航栏依次填写配置：
1. **源文件** — 选择 ZIP 包路径和输出目录
2. **包名** — 设置目标包名和保留模块
3. **项目** — 设置项目名称和版本
4. **模块映射** — 配置模块名前缀替换或精确映射
5. 点击侧边栏底部「开始处理」按钮
...
需提前编辑 `src/main/resources/setting.properties`：
